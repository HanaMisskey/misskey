	...
	<FormSlot>
		...
		<div class="_gaps_s">
			...
			<MkFolder v-if="matchQuery([i18n.ts._hana.crossRenoteAccountLimit, 'crossRenoteAccountLimit'])">
				...
				<template #suffix>
					<span v-if="role.policies.crossRenoteAccountLimit.useDefault" :class="$style.useDefaultLabel">{{ i18n.ts._role.useBaseValue }}</span>
					<span v-else>{{ role.policies.crossRenoteAccountLimit.value }}</span>
					<span :class="$style.priorityIndicator"><i :class="getPriorityIcon(role.policies.crossRenoteAccountLimit)"></i></span>
				</template>
				<div class="_gaps">
					<MkSwitch v-model="role.policies.crossRenoteAccountLimit.useDefault" :readonly="readonly">
						<template #label>{{ i18n.ts._role.useBaseValue }}</template>
					</MkSwitch>
					<MkInput v-model="role.policies.crossRenoteAccountLimit.value" :disabled="role.policies.crossRenoteAccountLimit.useDefault" type="number" :readonly="readonly">
					</MkInput>
					<MkRange v-model="role.policies.crossRenoteAccountLimit.priority" :min="0" :max="2" :step="1" easing :textConverter="(v) => v === 0 ? i18n.ts._role._priority.low : v === 1 ? i18n.ts._role._priority.middle : v === 2 ? i18n.ts._role._priority.high : ''">
						<template #label>{{ i18n.ts._role.priority }}</template>
					</MkRange>
				</div>
			</MkFolder>
