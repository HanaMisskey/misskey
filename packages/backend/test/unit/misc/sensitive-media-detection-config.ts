...
describe('センシティブ判定の接続設定の継承', () => {
	...
	test('ファイルと保存値が未指定なら組み込みの既定値を使う', () => {
		expect(getSensitiveMediaDetectionConfig()).toMatchObject({
			apiUrl: null, useProxy: true, timeout: 60000, maxImagesPerRequest: 4,
		});
	});

	test('null の保存値はファイルを継承し、false もそのまま使う', () => {
		expect(getSensitiveMediaDetectionConfig({ ...config, useProxy: false }, {
	...
	test('DB の指定項目だけを上書きし、false・旧既定値も明示値として扱う', () => {
		expect(getSensitiveMediaDetectionConfig(config, {
	...
	test.each([
		...
		expect(getSensitiveMediaDetectionConfig(config, { sensitiveMediaDetectionApiUrl: apiUrl }).apiUrl).toBe(expected);
	});
});
