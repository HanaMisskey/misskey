...
import type { MiSystemWebhook, SystemWebhooksRepository } from '@/models/mikroorm/_.js';
...
export class SystemWebhookEntityService {
	...
	public async pack(
		...
		const webhook = typeof src === 'object'
			? src
			: opts?.webhooks.get(src) ?? await this.systemWebhooksRepository.findOneOrFail({ id: src });
	...
	public async packMany(src: MiSystemWebhook['id'][] | MiSystemWebhook[]): Promise<Packed<'SystemWebhook'>[]> {
		...
		if (ids.length > 0) {
			webhooks.push(
				...await this.systemWebhooksRepository.find({ id: { $in: ids } }),
			);
		}
