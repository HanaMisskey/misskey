...
import type { DriveFilesRepository, PagesRepository, PageLikesRepository } from '@/models/mikroorm/_.js';
import { awaitAll } from '@/misc/prelude/await-all.js';
import type { Packed } from '@/misc/json-schema.js';
import type { } from '@/models/mikroorm/Blocking.js';
import type { MiUser } from '@/models/mikroorm/User.js';
import type { MiPage } from '@/models/mikroorm/Page.js';
...
export class PageEntityService {
	...
	public async pack(
		...
		const page = typeof src === 'object' ? src : await this.pagesRepository.findOneOrFail({ id: src });

		const attachedFiles: Promise<MiDriveFile | null>[] = [];
		const collectFile = (xs: any[]) => {
			for (const x of xs) {
				if (x.type === 'image') {
					attachedFiles.push(this.driveFilesRepository.findOne({
						id: x.fileId,
						userId: page.userId,
					}));
				}
		...
		if (migrated) {
			this.pagesRepository.nativeUpdate(page.id, {
		...
		return await awaitAll({
			...
			isLiked: meId ? !!await this.pageLikesRepository.findOne({
				pageId: page.id,
				userId: meId,
			}) : undefined,
		});
