...
import type { MiNote } from '@/models/mikrooorm/Note.js';
import type { UsersRepository, NotesRepository, FollowingsRepository, PollsRepository, PollVotesRepository, NoteReactionsRepository, ChannelsRepository, MiMeta } from '@/models/mikrooorm/_.js';
...
export class NoteEntityService implements OnModuleInit {
	...
	public async pack(
		...
		const reactions = this.reactionService.convertLegacyReactions(
			this.reactionsBufferingService.mergeReactions(note.reactions, bufferedReactions.deltas ?? {}),
		);
		...
		const channel = note.channelId
			? note.channel
				? note.channel
				: await this.channelsRepository.findOne({ id: note.channelId })
			: null;
	...
	public async packMany(
		...
		if (meId) {
			...
			const myReactions = idsNeedFetchMyReaction.size > 0 ? await this.noteReactionsRepository.find({
				userId: meId,
