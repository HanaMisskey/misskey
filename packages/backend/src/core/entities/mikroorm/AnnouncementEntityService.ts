...
import type { AnnouncementsRepository, AnnouncementReadsRepository, MiAnnouncement, MiUser } from '@/models/mikroorm/_.js';
...
export class AnnouncementEntityService {
	...
	public async pack(
		...
		const announcement = typeof src === 'object'
			? src
			: await this.announcementsRepository.findOneOrFail({ id: src }) as MiAnnouncement & { isRead?: boolean | null };
		...
		if (me && announcement.isRead === undefined) {
			...
			announcement.isRead = readCount > 0;
		}
