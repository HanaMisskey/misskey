...
import type { GalleryLikesRepository, GalleryPostsRepository } from '@/models/mikrooorm/_.js';
import { awaitAll } from '@/misc/prelude/await-all.js';
import type { Packed } from '@/misc/json-schema.js';
import type { MiUser } from '@/models/mikrooorm/User.js';
import type { MiGalleryPost } from '@/models/mikrooorm/GalleryPost.js';
...
export class GalleryPostEntityService {
	...
	public async pack(
		...
		const post = typeof src === 'object' ? src : await this.galleryPostsRepository.findOneOrFail({ id: src });
		...
		const isLiked = meId
			? (await this.galleryLikesRepository
				...
				.andWhere('like.userId = :userId', { userId: meId })
				.getRawOne()) !== undefined
			: undefined;
