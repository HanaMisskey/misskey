...
import type { MiLocalUser, MiPartialLocalUser, MiPartialRemoteUser, MiRemoteUser, MiUser } from '@/models/mikroorm/User.js';
import {
	birthdaySchema,
	descriptionSchema,
	localUsernameSchema,
...
export class UserEntityService implements OnModuleInit {
	...
	constructor(
		...
		@InjectRepository(UserSecurityKeysRepository)
		private userSecurityKeysRepository: UserSecurityKeysRepository,

		@InjectRepository(FollowingsRepository)
		private followingsRepository: FollowingsRepository,

		@InjectRepository(FollowRequestsRepository)
		private followRequestsRepository: FollowRequestsRepository,

		@InjectRepository(BlockingsRepository)
		private blockingsRepository: BlockingsRepository,

		@InjectRepository(MutingsRepository)
		private mutingsRepository: MutingsRepository,

		@InjectRepository(RenoteMutingsRepository)
		private renoteMutingsRepository: RenoteMutingsRepository,

		@InjectRepository(NoteUnreadsRepository)
		private noteUnreadsRepository: NoteUnreadsRepository,

		@InjectRepository(UserNotePiningsRepository)
		private userNotePiningsRepository: UserNotePiningsRepository,

		@InjectRepository(UserProfilesRepository)
		private userProfilesRepository: UserProfilesRepository,

		@InjectRepository(UserMemoRepository)
	...
	public async getRelation(me: MiUser['id'], target: MiUser['id']): Promise<UserRelation> {
		const [
			following,
			isFollowedCount,
			hasPendingFollowRequestFromYouCount,
			hasPendingFollowRequestToYouCount,
			isBlockingCount,
			isBlockedCount,
			isMutedCount,
			isRenoteMutedCount,
		] = await Promise.all([
			this.followingsRepository.findOne({ followerId: me, followeeId: target }),
			this.followingsRepository.count({ followerId: target, followeeId: me }),
			this.followRequestsRepository.count({ followerId: me, followeeId: target }),
			this.followRequestsRepository.count({ followerId: target, followeeId: me }),
			this.blockingsRepository.count({ blockerId: me, blockeeId: target }),
			this.blockingsRepository.count({ blockerId: target, blockeeId: me }),
			this.mutingsRepository.count({ muterId: me, muteeId: target }),
			this.renoteMutingsRepository.count({ muterId: me, muteeId: target }),
		]);
		...
		return {
			...
			isFollowed: isFollowedCount > 0,
			hasPendingFollowRequestFromYou: hasPendingFollowRequestFromYouCount > 0,
			hasPendingFollowRequestToYou: hasPendingFollowRequestToYouCount > 0,
			isBlocking: isBlockingCount > 0,
			isBlocked: isBlockedCount > 0,
			isMuted: isMutedCount > 0,
			isRenoteMuted: isRenoteMutedCount > 0,
		};
	}
	...
	public async getRelations(me: MiUser['id'], targets: MiUser['id'][]): Promise<Map<MiUser['id'], UserRelation>> {
		const [
			...
			this.followingsRepository.find({ followerId: me })
				.then(f => new Map(f.map(it => [it.followeeId, it]))),
			this.followingsRepository.find({ followeeId: me }, { fields: ['followerId'] })
				.then(it => it.map(it => it.followerId)),
			this.followRequestsRepository.find({ followerId: me }, { fields: ['followeeId'] })
				.then(it => it.map(it => it.followeeId)),
			this.followRequestsRepository.find({ followeeId: me }, { fields: ['followerId'] })
				.then(it => it.map(it => it.followerId)),
			this.blockingsRepository.find({ blockerId: me }, { fields: ['blockeeId'] })
				.then(it => it.map(it => it.blockeeId)),
			this.blockingsRepository.find({ blockeeId: me }, { fields: ['blockerId'] })
				.then(it => it.map(it => it.blockerId)),
			this.mutingsRepository.find({ muterId: me }, { fields: ['muteeId'] })
				.then(it => it.map(it => it.muteeId)),
			this.renoteMutingsRepository.find({ muterId: me }, { fields: ['muteeId'] })
				.then(it => it.map(it => it.muteeId)),
		]);
	...
	public async getNotificationsInfo(userId: MiUser['id']): Promise<{
		...
		if (!latestReadNotificationId) {
			...
			response.unreadCount = Math.max(latestNotificationIdsRes.length - 1, 0);
		}
	...
	public async getHasPendingReceivedFollowRequest(userId: MiUser['id']): Promise<boolean> {
		const count = await this.followRequestsRepository.count({
			followeeId: userId,
		});
	...
	public async pack<S extends 'MeDetailed' | 'UserDetailedNotMe' | 'UserDetailed' | 'UserLite' = 'UserLite'>(
		...
		const user = typeof src === 'object' ? src : await this.usersRepository.findOneOrFail({ id: src });
		...
		const profile = isDetailed
			? (opts.userProfile ?? await this.userProfilesRepository.findOneOrFail({ userId: user.id }))
			: null;
		...
		if (isDetailed && meId) {
			if (opts.userMemos) {
				...
				memo = await this.userMemosRepository.findOne({ userId: meId, targetUserId: user.id })
		...
		if (isDetailed) {
			if (opts.pinNotes) {
				...
				pins = await this.userNotePiningsRepository.find(
					{ userId: user.id },
					{ populate: ['note'], orderBy: { id: 'DESC' } },
				);
			}
		}
		...
		const packed = {
			...
			...(isDetailed ? {
				...
				publicReactions: this.isLocalUser(user) ? profile!.publicReactions : false,
			...
			...(isDetailed && (isMe || iAmModerator) ? {
				...
				securityKeys: profile!.twoFactorEnabled
					? this.userSecurityKeysRepository.count({ userId: user.id }).then(result => result >= 1)
					: false,
			} : {}),
	...
	public async packMany<S extends 'MeDetailed' | 'UserDetailedNotMe' | 'UserDetailed' | 'UserLite' = 'UserLite'>(
		...
		if (options?.schema !== 'UserLite') {
			profilesMap = await this.userProfilesRepository.find({ userId: { $in: _userIds } })
				.then(profiles => new Map(profiles.map(p => [p.userId, p])));

			const meId = me ? me.id : null;
			if (meId) {
				userMemos = await this.userMemosRepository.find({ userId: meId })
				...
				if (_userIds.length > 0) {
					...
					pinNotes = await this.userNotePiningsRepository.find(
						{ userId: { $in: _userIds } },
						{ populate: ['note'] },
					).then(pinsNotes => {
