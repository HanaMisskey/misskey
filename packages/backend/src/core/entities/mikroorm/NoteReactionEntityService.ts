...
export class NoteReactionEntityService implements OnModuleInit {
	...
	public async pack(
		...
		const reaction =
			typeof src === 'object'
				? src
				: await this.noteReactionsRepository.findOneOrFail({
					where: { id: src },
				  });
		...
		return {
			...
			user:
				hints?.packedUser ??
				(await this.userEntityService.pack(reaction.user ?? reaction.userId, me)),
	...
	public async packMany(
		...
		return Promise.all(
			reactions.map((reaction) =>
				this.pack(reaction, me, opts, { packedUser: _userMap.get(reaction.userId) }),
			),
		);
	}
}
