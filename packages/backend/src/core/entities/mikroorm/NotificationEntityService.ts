...
import type { FollowRequestsRepository, NotesRepository, MiUser, UsersRepository } from '@/models/mikroorm/_.js';
import { awaitAll } from '@/misc/prelude/await-all.js';
import type { MiGroupedNotification, MiNotification } from '@/models/mikroorm/Notification.js';
import type { MiNote } from '@/models/mikroorm/Note.js';
...
export class NotificationEntityService implements OnModuleInit {
	...
	public async pack(
		...
		const noteIfNeed = needsNote ? (
			hint?.packedNotes != null
				...
				: await this.noteEntityService.pack(notification.noteId, { id: meId }, { detail: true })
		) : undefined;
		...
		if (notification.type === 'reaction:grouped') {
			const reactions = (await Promise.all(
				notification.reactions.map(async reaction => {
					...
					return { user, reaction: reaction.reaction };
				}),
			...
			const users = (await Promise.all(
				notification.userIds.map(userId => {
					...
					return packedUser ?? this.userEntityService.pack(userId, { id: meId });
				}),
			)).filter(x => x != null);
		...
		return await awaitAll({
			...
			...(notification.type === 'roleAssigned' ? { role } : {}),
			...(notification.type === 'followRequestAccepted' ? { message: notification.message } : {}),
			...(notification.type === 'achievementEarned' ? { achievement: notification.achievement } : {}),
			...(notification.type === 'exportCompleted' ? { exportedEntity: notification.exportedEntity, fileId: notification.fileId } : {}),
			...(notification.type === 'app' ? { body: notification.customBody, header: notification.customHeader, icon: notification.customIcon } : {}),
		});
	}
	...
	async #packManyInternal <T extends MiNotification | MiGroupedNotification>(
		...
		const notes = noteIds.length > 0 ? await this.notesRepository.find({
			where: { id: { $in: noteIds } }, // MikroORM対応
		...
		const users = userIds.length > 0 ? await this.usersRepository.find({
			where: { id: { $in: userIds } }, // MikroORM対応
		}) : [];
		...
		if (followRequestNotifications.length > 0) {
			const reqs = await this.followRequestsRepository.find({
				where: { followerId: { $in: followRequestNotifications.map(x => x.notifierId) } }, // MikroORM対応
			});
	...
	async #filterValidNotifier <T extends MiNotification | MiGroupedNotification> (
		...
		const notifierIds = notifications
			.map(notification => 'notifierId' in notification ? notification.notifierId : null)
			.filter(x => x != null);

		const notifiers = notifierIds.length > 0
			? await this.usersRepository.find({ where: { id: { $in: notifierIds } } }) // MikroORM形式に変更
			: [];
