...
import type { BlockingsRepository } from '@/models/mikroorm/_.js';
import { awaitAll } from '@/misc/prelude/await-all.js';
import type { Packed } from '@/misc/json-schema.js';
import type { MiBlocking } from '@/models/mikroorm/Blocking.js';
...
export class BlockingEntityService {
	...
	public async pack(
		...
		const blocking = typeof src === 'object'
			? src
			: await this.blockingsRepository.findOneOrFail({ id: src as MiBlocking['id'] });

		return await awaitAll({
			id: blocking.id,
			createdAt: this.idService.parse(blocking.id).date.toISOString(),
			blockeeId: blocking.blockeeId,
			blockee: hint?.blockee ?? this.userEntityService.pack(blocking.blockeeId as MiUser['id'], me, {
	...
	public async packMany(
		...
		return Promise.all(blockings.map(blocking => this.pack(blocking, me, { blockee: _userMap.get(blocking.blockeeId as MiUser['id']) })));
	}
}
